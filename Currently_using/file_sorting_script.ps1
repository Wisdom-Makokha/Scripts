...
if ($files.Count -ne $initialFileCount) {
    ...
    $categories = @{}
    try {
        [hashtable]$categories = & "C:\Users\Dev Ark\Downloads\Script\Currently_using\read_hash_from_file.ps1" -filePath $categoriesFilePath -ErrorAction Stop
    }
    catch {
        try {
    ...
    if ($sortStats['Other'] -gt $initialFileCount ) {
        ...
        & "C:\Users\Dev Ark\Downloads\Script\Currently_using\print_hash_to_file_script.ps1" -filePath $categoriesFilePath -hash_to_print $categories
        & "C:\Users\Dev Ark\Downloads\Script\Currently_using\print_hash_to_file_script.ps1" -filePath $categoriesBackUpFilePath -hash_to_print $categories
    }
}
else {
